            ...
            <div class="container-padding-body-wrapper">
                ...
                <div style="margin-top: 26px;">
                    <el-table
                        ...
                        <el-table-column align="left" label="安全域名称" min-width="160" prop="name" />
                        <el-table-column align="left" label="接口集合" min-width="160" prop="interfaceCollection" />
        ...
        <el-drawer v-if="addDrawerFlag" custom-class="behaviorDrawer" :title="dialogTitle" :visible.sync="addDrawer" :direction="direction" :before-close="closeDialog" :wrapperClosable="false" @mousedown.native="handleWrapperMousedown" @mouseup.native="handleWrapperMouseup">
			<div>
                <el-form class="formDrawer" ref="form" :model="form" :rules="rules">
                    ...
                    <el-form-item label="引用接口" prop="interfaceCollection">
                        <div style="border: 1px solid #DCDFE6;overflow: hidden;">
                            <div style="width: 49.8%;float: left;border-right: 1px solid #DCDFE6;">
                                ...
                                <div style="padding: 8px 12px;">
                                    <el-input
                                        placeholder="请输入内容"
                                        v-model="searchInput">
                                        <i slot="suffix" class="el-input__icon el-icon-search" @click="searchSafeZoneFunc"></i>
                                    </el-input>
                                    <el-checkbox-group v-model="form.interfaceCollection">
                                        <el-checkbox v-for="item in ethList" :label="item" :key="item.id" style="display: block;">
                                            {{ item.name }}
                                        </el-checkbox>
                                    </el-checkbox-group>
                                </div>
                            ...
                            <div style="width: 50%;float: left">
                                <div style="border-bottom: 1px solid #DCDFE6;padding: 2px 12px;">
                                    已选：{{ form.interfaceCollection.length }}项
                                </div>
                                <div style="padding: 8px 12px;">
                                    <div v-for="item, index in form.interfaceCollection" :key="index">
                                        <span>{{ item.name }}</span>
                                        <span style="float: right;cursor: pointer;" @click="deleteChoosedSafeZoneFunc(item, index)">
    ...
    export default {
        ...
        created() {
			...
			if (tmpUserOperationUrl) {
				// 新增
				let tmp1 = tmpUserOperationUrl.includes('/resourceObj/safeZone/add')
				if (tmp1) {
                ...
                // 删除
                let tmp2 = tmpUserOperationUrl.includes('/resourceObj/safeZone/delete')
				if (tmp2) {
                ...
                // 编辑
                let tmp3 = tmpUserOperationUrl.includes('/resourceObj/safeZone/update')
				if (tmp3) {
        ...
        methods: {
            ...
            async getTableData() {
                ...
                let res = await getSafeZoneList(tmpdata)
                if (res.code === 0) {
            ...
            deleteFunc() {
                this.$confirm(`确定删除所选安全域吗?`, '提示', {
                    ...
                    beforeClose: (action, ctx, close) => {
                        ...
                        deleteSafeZone(tmpdata).then(res => {
                            if (res.code === 0) {
                                this.$message({
                                    type: 'success',
                                    message: res.msg
            ...
            deleteChoosedSafeZoneFunc(item, index) {
                const itemList = this.form.interfaceCollection.indexOf(item)
				if (itemList !== -1) {
					this.form.interfaceCollection.splice(index, 1)
				}
            },
            ...
            addFunc() {
                ...
                this.dialogTitle = '添加安全域'
            },

            editFunc(row) {
                this.getSafeZoneDetailFunc(row.id)
                this.isEdit = true
                this.addDrawerFlag = true
                this.$nextTick(() => {
                    this.addDrawer = true
                })
                this.dialogTitle = '编辑安全域'
            },

            async getSafeZoneDetailFunc(id) {
                let res = await getSafeZoneDetail(id)
                if (res.code === 0) {
                    if (res.data.resourceSafeZone) {
                        res.data.resourceSafeZone.interfaceCollection = JSON.parse(res.data.resourceSafeZone.interfaceCollection)
                        if (res.data.resourceSafeZone.interfaceCollection && res.data.resourceSafeZone.interfaceCollection.length > 0) {
                            let tmpSafeZoneArray = res.data.resourceSafeZone.interfaceCollection
                            var newArr = this.ethList.filter(item => {
                                // 将arr的所有id集合的数组赋值给临时数组,
                                //map()会返回一个新数组,并且不会影响原数组
                                let arrlist = tmpSafeZoneArray.map(item2 => item2)
                                // 返回所有临时数组中没有item.id的item
                                return arrlist.includes(item.name)
                            })
                            res.data.resourceSafeZone.interfaceCollection = newArr
                        }
                    }
                    this.form = res.data.resourceSafeZone
                }
            },
            ...
            enterDialog() {
                this.$refs.form.validate(async valid => {
					if (valid) {
                        this.loading = true
                        if (this.isEdit) {
                            this.form.interfaceCollection = JSON.stringify(this.form.interfaceCollection.map(item2 => item2.name))
                            let res = await updateSafeZone(this.form)
                            if (res.code === 0) {
                                this.$message({
                                    type: 'success',
                                    message: res.msg,
                            ...
                            this.form.interfaceCollection = JSON.stringify(this.form.interfaceCollection.map(item2 => item2.name))
                            let res = await addSafeZone(this.form)
                            if (res.code === 0) {
                                this.$message({
                                    type: 'success',
                                    message: res.msg,
